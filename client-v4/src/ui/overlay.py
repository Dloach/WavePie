...
class OverlayUI:
    ...
    def confirm_and_exit(self):
        ...
        # 保存选中扇区用于闪烁
        self._flash_idx = self._selected
        self._confirm_t = time.monotonic()
        self._step()

    def _step(self):
        t = time.monotonic() - self._confirm_t
        flash_dur = 0.2   # 高亮闪烁阶段
        fade_dur  = 0.3   # 渐隐阶段
        total = flash_dur + fade_dur
        if t >= total:
            self.deactivate(); return
        # ── 阶段1: 高亮闪烁 (0 → 0.2s) ──
        if t < flash_dur:
            blink_on = (int(t * 1000) // 50) % 2 == 0
            self._flash_sector(self._flash_idx, blink_on)
            self.root.attributes("-alpha", 0.85)
        # ── 阶段2: 渐隐 (0.2 → 0.5s) ──
        else:
            ft = t - flash_dur
            self._flash_sector(self._flash_idx, False)  # 关闪烁
            self.root.attributes("-alpha", max(0.01, 0.85 * (1.0 - ft / fade_dur)))
        self.root.after(16, self._step)

    def _flash_sector(self, idx, on):
        """闪烁选中扇区: on=True 金色高亮, on=False 关闭."""
        if "sec_fills" not in self._ids or idx < 0 or idx >= len(self._ids["sec_fills"]):
            return
        ox, oy, mr = self._cx, self._cy, self._menu_r
        fill_id = self._ids["sec_fills"][idx]
        if on:
            self._canvas.itemconfig(fill_id, fill="#FFD700", stipple="")
            self._canvas.coords(fill_id, ox - mr, oy - mr, ox + mr, oy + mr)
        else:
            self._canvas.itemconfig(fill_id, fill="#4488FF", stipple="gray50")
            self._canvas.coords(fill_id, 0, 0, 1, 1)
