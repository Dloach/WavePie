...
## D-005 | 2026-05-14 | client-v1 归档，经验文档化

**背景：** client-v1 已完成全部目标任务（PC 端原型 + ESP32 固件 + BLE 硬件联调），
准备进入 V2 开发。开发过程中积累了大量调试经验和架构教训，需要系统化记录。

**决策：**
1. `docs/postmortem-v1.md` 作为 V1 经验总结文档，V2 开发前必须阅读
2. 文档覆盖：架构验证、关键 Bug 及根因、硬件经验、V2 建议
3. SESSION_STATE.md 标记 client-v1 为已归档
4. 仓库末尾 tagged v0.0.1 指向 client-v1 最终状态

**理由：** 避免 V2 重复 V1 踩过的坑。闭包捕获、Enum 比较、tkinter 线程安全、
BLE 高频调度等 bug 都有明确的根治方案。

**后续影响：**
- V2 开发前需完整阅读 `docs/postmortem-v1.md`
- 该文档应随 V2 开发持续更新
